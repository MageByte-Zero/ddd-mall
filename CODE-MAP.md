...
| L4 | `ProductApplication`/`InventoryApplication`/`PaymentApplication` 启动类（@EnableDiscoveryClient + @MapperScan）、3 份 application.yml（discovery 开 / config 关 / flyway 关）、3 个模块 spring-boot-maven-plugin；落盘 commit 44e0da8（代码）+ a6d628a（yml 注释读者化）。4 应用注册 Nacos、mall-order Flyway V1 迁移实测通过 | — | — |
