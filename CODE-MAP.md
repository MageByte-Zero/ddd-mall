...
### mall-inventory (支撑域) — 启动类 L4 / 四层包 L5 / 最小 AT 参与者 L10 / 归还反向能力 L11 / 预占模型待 L12

#### domain/
- `Inventory`（聚合根，最小形态）— L10：`reconstitute(id, skuCode, skuName, totalStock, availableStock, createdAt, updatedAt)`；`deduct(int quantity)` 守卫（数量必须为正、可售不得为负）；**L11 加对称的 `release(int quantity)` 守卫**（数量必须为正、**可售不得还到总库存之上**——越界即疑似重复释放，直接拒绝而不是静默累加）；仍**不含**预占/释放双字段模型与乐观锁（L12 重写）
- `InventoryDomainException`（领域异常）— L10：接口层翻译为 HTTP 422
- `InventoryRepository`（仓储接口）— L10：`findBySkuCode(String)`；`deduct(String, int)` 返回条件 UPDATE 影响行数；L11 加 `int release(String skuCode, int quantity)`

#### infrastructure/persistence/
- `InventoryDO`（@TableName t_inventory；@Version/@TableLogic，乐观锁 L12 启用）/ `InventoryMapper extends BaseMapper<InventoryDO>` — L10
- `InventoryRepositoryImpl`（@Repository）— L10：`deduct` 用 `lambdaUpdate().setSql("available_stock = available_stock - n").ge(availableStock, n)` 条件更新，并发不扣负；**L11 加 `release`**：`setSql("available_stock = available_stock + n")` + `.apply("available_stock + n <= total_stock")`，把"还完不越界"的护栏同样下沉到 SQL——领域守卫拦单线程越界，这条 WHERE 拦并发越界，两者缺一不可

#### application/
- `InventoryApplicationService`（@Service）— L10：`deduct(skuCode, quantity)` 标 `@Transactional`，是 AT 分支事务边界；读聚合走守卫 → 条件 UPDATE 兜底 → 影响 0 行抛库存不足（并发冲突）。L11 加 `release(skuCode, quantity)`：同样 `@Transactional` 登记为分支；领域守卫 → 条件 UPDATE → 影响 0 行抛"归还越界（疑似重复释放或并发冲突）"，异常经 422 回到订单 BC 触发取消用例回滚

#### interfaces/rest/
- `InventoryController`（@RestController `/api/inventories`）— L10：`POST /api/inventories/deductions` → 200 + `Result<Void>`；库存不足 422。L11 加 `POST /api/inventories/releases` → 200 + `Result<Void>`；归还越界 422
- `DeductInventoryRequest`（record：skuCode/quantity，JSR-380）— L10
- `ReleaseInventoryRequest`（record：skuCode/quantity，JSR-380；quantity `@Min(1)`）— L11
...
- 测试 `InventoryTest`（4 用例：扣减/不足/非法数量/重组脏数据）+ `InventoryApplicationServiceTest`（3 用例：真实 MySQL 条件扣减，@Transactional 回滚）— L10；**L11 各加 4 和 3 个用例**：`InventoryTest` 补归还成功/归还越界/非法归还数量/扣减后归还回到原值，`InventoryApplicationServiceTest` 补归还生效/越界回滚/未知 SKU
...
| mall-order | mall-inventory | 同步 HTTP（Feign，同一 Feign 接口的反向端点） | `POST /api/inventories/releases` | Seata AT 全局事务内（取消用例，与下单对称） | L11（防腐层加厚待 L17/L19） |
